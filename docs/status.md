# 作業ステータス
最終更新: 2026-08-24（Rangers 決済まわりの総点検・修正完了。ともくん自身の手動確認タスク3件が残っている ↓下記参照）

---

## ⚠️ ともくんへの残タスク（要手動対応・忘れずに）（2026-08-24）

決済(Stripe)の総点検がコード側は完了。残っているのは以下3点で、いずれもダッシュボード操作が必要なためともくん自身での対応が必要。詳細は `docs/PAYMENT_TEST_REPORT_2026-08-22.txt` の11〜12節参照。

1. **Vercel Production環境変数の確認** — 本番用Stripeキー（`STRIPE_SECRET_KEY`等）がVercelダッシュボードのProduction環境に正しく設定されているか確認する
2. **Stripeライブモードでの本番Webhookエンドポイント登録確認** — Stripeダッシュボードをライブモードに切り替えたうえで、`https://swim-os-seven.vercel.app/api/stripe/webhook` 宛のWebhookエンドポイントが登録されているか確認する（テストモードには既に正しく登録済み・タイプミスで壊れていた古いエンドポイントは削除済み）
3. **（出金まで行う場合のみ）本人確認書類の本番再提出** — マウントリバー水泳クラブ等のConnectアカウントで、テストモードの仮アップロードでは`payouts_enabled`が有効化されなかった。実際にチームが銀行口座へ出金するには、本番のStripeダッシュボードで本人確認書類を実画像で再提出する必要がある
