# 作業ステータス
最終更新: 2026-08-26（Rangers 決済まわりの総点検・修正完了。残タスクは全て「本番切り替え時」でよいことを確認済み ↓下記参照）

---

## ⚠️ ともくんへの残タスク（本番切り替え時に対応・今は不要）（2026-08-24、2026-08-26に優先度を再整理）

決済(Stripe)の総点検はコード側・テスト側ともに完了。残っている以下2点は、いずれも「実際に本番へ切り替えるタイミング」で対応すればよく、現時点で急いで対応する必要はない。詳細は `docs/PAYMENT_TEST_REPORT_2026-08-22.txt` の11〜13節参照。

1. **Vercel Production環境変数の確認**（本番切り替え時） — 本番用Stripeキー（`STRIPE_SECRET_KEY`等）がVercelダッシュボードのProduction環境に正しく設定されているか確認する
2. **Stripeライブモードでの本番Webhookエンドポイント登録確認**（本番切り替え時） — Stripeダッシュボードをライブモードに切り替えたうえで、`https://swim-os-seven.vercel.app/api/stripe/webhook` 宛のWebhookエンドポイントが登録されているか確認する（テストモードには既に正しく登録済み・タイプミスで壊れていた古いエンドポイントは削除済み）

~~3. 本人確認書類の本番再提出~~ → **対応不要と判明**。これはテストモードのConnectアカウント（マウントリバー水泳クラブ等）の話であり、テストモードと本番(ライブ)モードのConnectアカウントは別物。本番移行時はコーチが実際の本人確認書類でオンボーディングをやり直すことになるため、テストモード側のこの状態は本番に一切引き継がれない。参加費の受け入れ・送金分配（`charges_enabled`）には影響がなく、テスト環境で実際の着金まで見たい場合にのみ意味がある任意対応。

---

## 完了 ✅: Rangers 決済(Stripe)まわりの総点検・不具合修正（2026-08-22〜24）

「支払い周りは一番大事なので、丁寧に時間をかけて」という依頼を受け、複数セッションにまたがって決済まわりを徹底的にレビュー・自動テスト・修正した。詳細・時系列の全記録は `docs/PAYMENT_TEST_REPORT_2026-08-22.txt` にまとめてある（本ファイルには要点のみ記載）。

### 発見・修正した重大な不具合（計6件）
1. 月謝Subscriptionの初回課金が確定処理漏れで一度も課金されていなかった
2. 2年目以降の年会費・2ヶ月目以降の月謝（更新分）がDBに一切記録されない（Stripeの請求期間計算ミス）
3. セッション参加費のカード決済が`off_session`パラメータの誤りで常に失敗していた
4. 初回課金失敗（無効なカード等）がWebhook未発火のため誰にも気づかれず永久に未収のままになる
5. 外部公開セッションでゲスト(非会員)が「配信対象」のデフォルト設定により常に参加登録できない
6. セッション詳細画面で管理者専用ボタンが一般メンバーにも見えていた（表示のみ、サーバー側は保護済み）

### 自動テストで正常動作を確認した項目
月謝・年会費の自動課金と翌年/翌月更新、回数券払い、現金払い、返金、決済失敗からの再試行、Connect未完了チームでの決済ブロック、Connect未設定チームでのゲスト決済、プラットフォーム手数料分配。

### 手動で実施した項目
Stripe Connectオンボーディング（ボット検知認証のため自動化不可）→ ともくんの操作で完了確認済み。

### 本番インフラの点検
本番URL宛に、設定時タイプミスの古いStripe Webhookエンドポイント（`/api/weebhooks/stripe`）が2026年5月から404を出し続けているのを発見し削除。正しいエンドポイントは別途正常稼働していたため実害はなし。

### 次にやること
上記「⚠️ ともくんへの残タスク」の3点のみ。コード側で洗い出せる範囲の決済関連対応はすべて完了。

---

## 完了 ✅: Rangers 全体コードレビュー（フロントエンド・バックエンド・DB・決済・セキュリティ）（2026-08-15）

ともくんから「今回触った部分を含め、アプリ全体を様々な観点から丁寧にレビューしてほしい」と依頼され、
フロントエンド・バックエンド・DB/RLS・Stripe決済・認証セッションの5観点で専門エージェントを並列起動して実施。
Supabase Advisorも併用。詳細な指摘一覧・裏取り根拠は `docs/for-reviewer/FULL_APP_REVIEW_2026-08-15.md` に記載。

### 結果
CRITICAL 1件・HIGH 8件・MEDIUM 13件・LOW 10件を検出し、ほぼ全て修正・本番反映済み。

### 特筆すべき発見・対応
- **CRITICAL**: `supabase/migrations/00025_test_users_cleanup.sql` にテストユーザーのパスワードが平文でハードコードされ本番に適用されていた。本番の`test5@example.com`のパスワードをローテーション済み（ともくん承認済み）。
- **調査依頼**: `lessons`/`bookings`/`reviews`/`schedule_requests`テーブルが個人レッスン予約マーケットプレイスだった初期構想の残骸と判明（過去レビューでも「未参照」と確認済みだったが、ともくんの指示で改めて依存関係・他アプリ(school-boost-ai)との関係を再調査 → 100%未使用と確認）。本番から削除済み（`00075_drop_legacy_marketplace_tables.sql`）。
- **DB**: `profiles`のStripe連携ID列・`messages`の既読更新ポリシーに、一般ユーザーが直接書き換えられる穴があり、トリガーで防止（`00074_review_2026-08-15_hardening.sql`）。チーム削除時に決済履歴がCASCADEで消える問題もNO ACTIONに変更して解消。スキーマドリフト（本番には存在するがマイグレーション履歴に記録がなかった列）も複数発見・補完。
- **認証**: レート制限が`X-Forwarded-For`ヘッダーの偽装で無効化できる問題を修正（先頭ではなく末尾のIPを採用）。
- **見送り**: CookieのhttpOnly化は`@supabase/ssr`のブラウザクライアント設計と衝突しセッションが壊れるリスクがあるため、ともくんと相談の上で今回は対応せず。

### Supabase「Password Leak Protection（漏洩パスワード保護）」について
ともくんから「RLSによるパスワード保護」という質問があり、これはRLS（行レベルセキュリティ）とは別機能（HaveIBeenPwned連携のパスワード強度チェック）であることを説明。**組織(ktomoyuki0227's Org)が現在Freeプランのため有効化不可**（Proプラン以上が必須と公式ドキュメントで確認済み）と判明。ともくんの回答: 「後日Proプランへアップグレードするので一旦Freeプランで進める」「最小パスワード長は既に8文字に設定済みなので対応不要」。バックログに記録済み（下記「バックログ」セクション参照）。

### 次にやること
- `git status`で変更ファイルを確認 → ともくんの確認後にcommit・push（このセッションではcommitしていない）
- L-3（stamp_purchasesの冪等性制約）は自然なユニークキーが無いため見送り。将来的に購入フォームへのidempotency key追加という機能追加が必要
- Supabase Advisor「漏洩パスワード保護」は上記の通りProプランアップグレード待ち
...
- [ ] **Proプランへアップグレード後、Supabase Dashboardで「Password Leak Protection（漏洩パスワード保護）」を有効化**（2026-08-15確認: 組織 ktomoyuki0227's OrgがFreeプランのため現状は有効化不可。Proプラン以上が必須。ダッシュボード Authentication → Sign In / Providers → Email 内のトグル。ともくんの手動操作が必要。なお最小パスワード長は既に8文字に設定済みのため対応不要）
