...
import { getOrCreateStripeProduct, getOrCreateMonthlyPrice, getOrCreateAnnualPrice } from "@/lib/stripe-helpers"
...
/**
 * チームの年会費額が変更された際、既にアクティブなStripe Subscriptionを
 * 全て新しい価格に切り替える。syncActiveSubscriptionsToNewPriceと対になる関数
 * （membership_type/価格作成関数/カラム名がannual側になるだけで挙動は同一）。
 */
export async function syncActiveAnnualSubscriptionsToNewPrice(
  admin: AdminClient,
  teamId: string,
  teamName: string,
  newAmount: number,
  ...
  try {
    const productId = await getOrCreateStripeProduct(teamId, teamName, currentProductId)
    newPriceId = await getOrCreateAnnualPrice(teamId, newAmount, productId, currentPriceId)
  } catch (err) {
    console.error("[syncActiveAnnualSubscriptionsToNewPrice] failed to prepare new price:", err)
    return { updated: 0, failed: activeMembers.length }
  }
  ...
  await mapWithConcurrency(activeMembers, SUBSCRIPTION_SYNC_CONCURRENCY, async (member) => {
    ...
    try {
      ...
      console.error(`[syncActiveAnnualSubscriptionsToNewPrice] failed for member ${member.id}:`, err)
      failed++
    }
  })
