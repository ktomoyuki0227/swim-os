...
export function PublicTeamView({ data }: { data: PublicTeamData }) {
  ...
  // Hero banner: prefer cover_image_url, then avatar_url, then gradient
  const heroBannerUrl = team.cover_image_url ?? team.avatar_url ?? null
  // Activity area: prefer team-level field, fallback to coach prefecture
  const activityArea = team.activity_area ?? coachPrefecture ?? null

  return (
    <div className="-mx-4 -mt-6 min-h-screen bg-[#f5f8fa]">
      {/* ── ヒーロー ── */}
