...
function CashRow({ reg }: { reg: CashRegistration }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg bg-white px-3 py-2.5">
      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#005F8C]/10 text-sm font-semibold text-[#005F8C]">
        {reg.swimmerName[0]}
      </div>
      <p className="flex-1 text-sm font-medium text-[#1a2332]">{reg.swimmerName}</p>
      {isFree ? (
        <Badge className="bg-[#edf0f4] text-[#475569] border-transparent text-sm">免除</Badge>
      ) : isCollected ? (
        <Badge className="bg-[#eaf7f0] text-[#0f8a4f] border-transparent text-sm">集金済み</Badge>
      ) : (
        <button
          onClick={() => startTransition(async () => { await markCashPaid(reg.id) })}
          disabled={isPending}
          className="rounded-lg bg-[#005F8C] px-3 py-1.5 text-sm font-medium text-white disabled:opacity-50 hover:bg-[#004E73] transition-colors"
        >
